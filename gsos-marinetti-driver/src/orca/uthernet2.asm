...
*=================================================
*
* Uthernet2.ASM - Uthernet2 link layer module
*
* Copyright (C) 1998-2002 Richard Bennett
* Copyright (c) 2005 Glenn Jones
* Copyright (c) 2004-2005 MagerValp
* Copyright (c) 2006-2019 Ewen Wannop
* Copyright (c) 2024 Stephen Heumann
* Code from ip65 MagerValp is licensed ala BSD
*
* Part Code Copyright (c) 2013, Oliver Schmidt <ol.sc@web.de>
* Modified to ORCA 16 bit code (c) 2014, Ewen Wannop
* All rights reserved. 
*
...
*                   and MagerValps ip65
* 2014.05.05 EW	  - Changes derived from w1500s code by Oliver Schmidt
*				    applied to original Uthernet I Link Layer 
* 2015.06.28 EW	  - Cleaned up code on working Uthernet II card 
* 2015.07.03 EW	  - Added choice of MTU option 
...
cfg_mac		dc h'00 08 dc 11 11 11'	; offset 14		; OUI of WIZnet
cfg_slot    dc i'4'					; offset 20
use_dhcp    dc i'0'					; offset 22
cfg_vers	dc i'wizversl'			; offset 24
            dc i'wizversh'			; offset 26
...
; wiz card registers
mode gequ $e0c084	; Mid byte patched at runtime
addr gequ $e0c085	; Mid byte patched at runtime
data gequ $e0c087	; Mid byte patched at runtime
...
wizversh		gequ $0205	;mmmm_mmmm_mmmm_bbbb
wizversl		gequ $2001	;sss0_0000_rrrr_rrrr ss-20=d,40=a,60=b,80=f,a0=r
...
* wizinterfacev():interfacev:integer;
* wizstartup():interfacev:integer;
* wizshutdown();
* wizmoduleinfo(infoblockptr:longword);
* wizgetpacket():packethandle:handle;
* wizsendpacket(datagramptr:longword,datagramlength:integer);
* wizconnect(conmsgflag:integer,usernameptr:longword,passwordptr:longword,displayptr:longword,conhandle:handle);
* wizreconstatus():reconstatus:boolean;
* wizreconnect(displayptr:longword);
* wizdisconnect(conmsgflag:integer,usernameptr:longword,passwordptr:longword,displayptr:longword,disconhandle:handle);
* wizgetvariables():variablesptr:longword
* wizconfigure(connecthandle,disconnecthandle);
* wizconfigfname(configfilenameptr:longword);
...
*-------------------------------------------------
wizstartup anop
	brl	suok		; dbgmsg wizstartup
	dc	i'$7771'	; ambed wizstartup
	str	'wizstartup'
suok nop
	nop
	
wizshutdown anop
	brl	sdok		; dbgmsg wizshutdown
	dc	i'$7771'	; ambed wizshutdown
	str	'wizshutdown'
sdok nop
	nop
...
*-------------------------------------------------
wizmoduleinfo anop
	brl	modok		; dbgmsg wizmoduleinfo(infoblockptr:longword);
	dc	i'$7771'	; ambed wizmoduleinfo(infoblockptr:longword)
	str	'wizmoduleinfo(infoblockptr:longword)'
modok nop
	nop
...
wizgetpacket anop
	brl	gpok		; dbgmsg wizgetpacket():packethandle:handle;
	dc	i'$7771'	; ambed wizgetpacket():packethandle:handle
	str	'wizgetpacket():packethandle:handle'
gpok nop
	nop
...
wizsendpacket anop
	...
	brl spok		; dbgmsg wizsendpacket(datagramptr:longword,datagramlength:integer);
	dc i'$7771'	; ambed wizsendpacket(datagramptr:longword,datagramlength:integer)
	str 'wizsendpacket(datagramptr:longword,datagramlength:integer)'
spok nop
	nop
...
;
	...
	ldx #0
wiz_1 anop			; Fixup address at location
	lda fixup,x
	sta ap
	lda	(ap)
	and	#$ff0f
...
; Max Segment Size Registers (MTU-40=MSS) Sockets 0: Assign 1460 as default
	...
	jsr set_addr
	sec
	lda	variables+lvmtu   ; mtu lo
	sbc	#40
	tax
	lda	variables+lvmtu+1 ; mtu hi
	sbc	#0
fixup30	sta >data
	txa
fixup31	sta >data
...
reg dc i'0'		; register
len	dc i'0'		; Packet length counter
adv dc i'0'		; Data pointer advancement
bas dc i'0'		; register base memory

err_return ds 2
temphandle ds 4
wiz_slot_offset dc i2'$0040'

fixup dc i'fixup01+1,fixup02+1,fixup03+1,fixup04+1,fixup05+1,fixup06+1'
	  dc i'fixup07+1,fixup08+1,fixup09+1,fixup10+1,fixup11+1,fixup12+1'
	  dc i'fixup13+1,fixup14+1,fixup16+1,fixup17+1,fixup18+1,fixup19+1'
	  dc i'fixup20+1,fixup22+1,fixup23+1,fixup24+1,fixup25+1,fixup26+1'
	  dc i'fixup27+1,fixup28+1,fixup29+1,fixup30+1,fixup31+1'
fixups gequ *-fixup
...
wizreconstatus anop
	brl	resok		; dbgmsg wizreconstatus
	dc    i'$7771'	; ambed wizreconstatus
	str   'wizreconstatus'
resok nop
	nop
...
wizreconnect anop
    brl	reok		; dbgmsg wizreconnect(displayptr:longword);
	dc i'$7771'		; ambed  wizreconnect(displayptr:longword)
	str 'wizreconnect(displayptr:longword)'
reok nop
	nop
...
wizdisconnect anop
	brl	diok                      	; dbgmsg wizdisconnect
	dc   i'$7771'	; ambed  wizdisconnect
	str  'wizdisconnect'
diok nop
	nop
...
linkstre str	'Shutdown UthernetII port'
...
wizgetvariables anop
	brl	gvok		; dbgmsg wizgetvariables
	dc    i'$7771'                 ; ambed  wizgetvariables
	str   'wizgetvariables'
gvok nop
	nop
...
wizconfigfname anop
	brl 	cfnok		; dbgmsg wizconfigfname(configfilenameptr:longword);
	dc    i'$7771'	; ambed  wizconfigfname(configfilenameptr:longword)
	str   'wizconfigfname(configfilenameptr:longword)'
cfnok nop
	nop
...
chck1 lda eth_outp+eth_data,x
	...
	lda	#$b405	; mtu lo = <1460 mtu hi = >1460
...
set_addrcmdreg0 anop
	ldy #$01		; Socket Command Register
set_addrsocket0 anop
...
ctltmp_00000001 anop	; uthernet2 title
           dc i2'9' ; pcount
...
letxtbox_00000014 anop
           ...
           dc c'F',i1'$fe,$ff,$00,$08,$01',c'C'
           dc i1'$00,$00,$01',c'B',i1'$ff,$ff'
           dc c'Happy 39th birthday to Apple!'
